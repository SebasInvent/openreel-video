  ...
  return (
    <div className="space-y-4">
      ...
      {hasAudio ? (
        <>
          {beatState.isAnalyzing ? (
            <div className="p-3 bg-bg-2 rounded-lg">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[10px] text-fg">{t("Analyzing beats...")}</span>
              </div>
          ...
          {beatState.beatAnalysis && (
            <div className="p-3 bg-green-500/10 border border-green-500/30 rounded-lg">
              <div className="flex items-center justify-between mb-2">
                <span className="text-[10px] text-fg-2">{t("Detected BPM")}</span>
                <span className="text-sm font-bold text-green-400">
                  {beatState.beatAnalysis.bpm}
                </span>
              </div>
              <div className="flex items-center justify-between mb-2">
                <span className="text-[10px] text-fg-2">{t("Confidence")}</span>
                <span className="text-[10px] text-fg">
                  {Math.round(beatState.beatAnalysis.confidence * 100)}%
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-[10px] text-fg-2">{t("Beat Markers")}</span>
                <span className="text-[10px] text-fg">
          ...
          {showAdvanced && (
            <div className="p-3 bg-bg-2 rounded-lg space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-[10px] text-fg-2">{t("Manual BPM")}</span>
                <div className="flex items-center gap-2">
        ...
        <div className="p-3 bg-bg-2 rounded-lg">
          <Text type="supporting" color="secondary" className="text-[10px] text-fg-3 text-center">{t("Select a video or audio clip to analyze beats")}</Text>
        </div>
      )}
